...
mv total_dvdl.dat dvdl_all.dat

last_step=$(tail -1 dvdl_all.dat |awk {'print $1'})
if [[ $last_step == "" ]]
      then
      cp dvdl.dat dvdl_all.dat
      last_step=$(tail -1 dvdl_all.dat |awk {'print $1'})
fi
...
complete_check=$(tail -1 $directory/dvdl.dat |awk '{print $1'})

if [[ $complete_check -eq $last_step ]];then
      mv dvdl_all.dat total_dvdl.dat
...
compare_step=$(head -2 $directory/dvdl.dat |awk 'NR==2 {print $1'})
...
cat dvdl_all.dat $directory/dvdl.dat > tmp && mv tmp dvdl_all.dat
